...
(function( $ ){
  ...
  $.fn.jresize = function(options) {
    ...
    return this.each(function() {
      ...
      // Test if jresize was previously run
      var jresize_loaded = false;      
      if ($image.parent().hasClass('jresize_viewport')) {
        jresize_loaded = true;        
      }
      ...
      if (jresize_loaded && destroy) {
        ...
        // jresize css or some user size or proportion changes.
      ...
      // Prepare image
      if (!jresize_loaded) {
        $image.hide().css('position','absolute').wrap('<div class="jresize_container"><div class="jresize_viewport"></div></div>');
      }
      ...
      var $loading = $('<div class="jresize_loading" />');
        ...
        $.extend($viewport, {
          ...
          observator: {
            ...
            notify: function(event_name, value) {
              ...
              $image.trigger('jresize_events',[$viewport]);
            },
            notify_all: function() {
              this.notify('jresize_image_width', $image.width());
              this.notify('jresize_image_height', $image.height());
            },
      ...
      if (/^data:image/.test(src)) {
        ...
        src = src + (src.search(/\?/)<0?'?':'&') + 'jresizerandom=' + (new Date()).getTime();
